...
namespace proto {
	...
	public:
		...
		class lock_handle : public non_copyable {
		public:
			...
			inline lock_handle(lock_handle &&) = default;
	...
	//	class proxy_task : public non_copyable {
	//	public:

	//		template < class _task_t >
	//		proxy_task(_task_t && task) :
	//			_task_handler(new task_handler < _task_t >(std::move(task)))
	//		{}
	...
	//			inline task_handler(_task_t && task) : 
	//				task(std::move(task))
	//			{}
	...
	//		enqueue_task(details::proxy_task < std::packaged_task < std::result_of_t < _ft_t(_args_t ...) >() > >(task));
	...
	//	std::queue < details::proxy_task >	_task_storage;
	//	std::mutex							_task_storage_mutex;
	//	std::atomic < bool >				_dispacher_running = false;
