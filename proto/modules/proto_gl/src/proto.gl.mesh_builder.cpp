...
namespace proto {
	namespace gl {
		...
		void mesh_builder::add_quad(
			...
			auto look_at = glm::lookAt (center, up, direction);

			vec3
				// top left
				v1 = look_at * vec4{ -hs, hs, .0F, 1.0F },
				// top right
				v2 = look_at * vec4{ hs, hs, .0F, 1.0F },
				// bottom left
				v3 = look_at * vec4{ hs, -hs, .0F, 1.0F },
				// bottom right
				v4 = look_at * vec4{ -hs, -hs, .0F, 1.0F };
